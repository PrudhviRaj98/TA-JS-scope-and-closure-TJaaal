...
function multiMap(arrVals, arrCallbacks) {
  return arrVals.reduce((acc,cv)=>{
    let valuesarr = arrCallbacks.map(n => n(cv))
    acc[cv] = valuesarr;
    return acc;
  } ,{})
...
function objOfMatchesWithArray(array1, array2, callbacks) {
  ...
  return array1.reduce ((acc,cv,index)=>{
    let valuesof = callbacks.reduce((acc,fn)=>{
      fn(acc)
    },cv)

    if (valuesof === array2[index]){
      acc[cv] = array2[index]
    }
    return acc;
  },{})
...
function objOfMatchesWithArray(array1, callback) {
  return array1.reduce((acc,cv)=>{
    let values = callback.map((fn)=> fn(cv))
    acc[cv] = values
  },{})
}
...
```js
function schedule(arrfn, arrsec) {
  if(arrfn.length !== arrsec){
    alert('Length is not same')
  }
  arrfn.forEach((fn,i) => {
    setTimeout(fn,arrsec[i] * 1000)
  })
